...
export function sampleEntries(): Omit<LedgerEntry, 'id'>[] {
  ...
  for (let m = 1; m <= thisMonth; m++) {
    out.push(
      ...
      { type: 'expense', date: `${year}-${pad(m)}-03`, description: 'Leather, thread and dye restock', category: 'Raw materials', amountMinor: (21000 + m * 800) * 100 },
      ...
      { type: 'expense', date: `${year}-${pad(m)}-28`, description: 'Workshop rent', category: 'Studio rent', amountMinor: 25000 * 100 }
    )
    if (m % 3 === 0) {
      out.push(
